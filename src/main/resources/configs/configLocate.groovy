...
//            (Constants.IGNORE): [pattern: ~/(?i)([_.~]+lock.*|[#_~].*|.*\bte?mp|.*\.class|.*\.pem|skipme.*)/, analysis: Constants.IGNORE_BUNDLE],
            office               : [pattern: ~/.*\.(?i)(accdb|do[ct]x?|ics|ods|odp|odt|ott|pages|pdf|pptx?|rtf|sxi|vsdx?|xmind|xlsx?)/, analysis: Constants.TRACK_BUNDLE, inherit: true],
            system               : [pattern: ~/.*(\.(?i)(bin|bundle|cab|deb|dmg|exe|jar|lib|md5|pkg|rpm|so)|(gcc.*))/, analysis: Constants.TRACK_BUNDLE, inherit: true],
            web                  : [pattern: ~/.*(?i)(html?)/, analysis: Constants.TRACK_BUNDLE, inherit: true],
            instructions         : [pattern: ~/.*(?i)(adoc|readme.*|man|md)/, analysis: Constants.TRACK_BUNDLE, inherit: true],
            techDev              : [pattern: ~/.*\.(?i)(c|css|go|groovy|gradle|java|javascript|js|php|py|schema|sh)/, analysis: Constants.TRACK_BUNDLE, inherit: true],
            config               : [pattern: ~/.*(?i)(\.cfg|config.*|\.pem|\.properties|\.xml|\.yaml)/, analysis: Constants.TRACK_BUNDLE, inherit: true],
            ...
            media                : [pattern: ~/.*\.(?i)(avi|bmp|gif|ico|jpe?g|kra|mp3|mpe?g|ogg|png|wav)/, analysis: Constants.TRACK_BUNDLE, inherit: true],
            communication        : [pattern: ~/.*\.(?i)(eml|vcf)/, analysis: Constants.TRACK_BUNDLE, inherit: true],
...
pathPatterns {
    folders = [
            ...
            workFolder     : [pattern: ~/(?i).*[\/\\](lucidworks|oconeco|work).*/, analysis: Constants.TRACK_BUNDLE, inherit: true],
            ...
            configFolder   : [pattern: ~/.*(\/etc\/).*/, analysis: Constants.TRACK_BUNDLE, inherit: true],
            manualsFolder  : [pattern: ~/.*(?i)\/(help|info|man)\/.*/, analysis: Constants.TRACK_BUNDLE, inherit: true],
    ]
    files = []
}
