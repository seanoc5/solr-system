...
namePatterns {
    folders = [
            ...
            content: [pattern: ~/(content)/, analysis: Constants.PARSE_BUNDLE],
            office : [pattern: ~/(?i).*(documents)/, analysis: Constants.PARSE_BUNDLE],
            techDev: [pattern: ~/.*(groovy|gradle|classes)/, analysis: Constants.PARSE_BUNDLE],
            system : [pattern: ~/(_global)/, analysis: Constants.PARSE_BUNDLE],
...
//            test   : [pattern: ~/(?i).*(test)/, analysis:],
    ...
    files = [
            ...
            index       : [pattern: ~/(bash.*|csv|groovy|ics|ipynb|java|lst|md|php|py|rdf|rss|scala|sh|tab|te?xt|tsv)/, analysis: Constants.PARSE_BUNDLE],
            office      : [pattern: ~/.*(accdb|docx?|ods|odp|odt|pdf|pptx?|ps|pub|rtf|txt|vsdx?|xmind|xlsx?)/, analysis: Constants.PARSE_BUNDLE],
            ...
            config      : [pattern: ~/.*(config.*|manifest.*|properties|xml|ya?ml)/, analysis: Constants.PARSE_BUNDLE],
            control     : [pattern: ~/.*(ignore|license.*|lock|pem)/, analysis: Constants.PARSE_BUNDLE],
            data        : [pattern: ~/.*(csv|jsonl?d?|lst|pbix|tab|tsv)/, analysis: Constants.PARSE_BUNDLE],
            media       : [pattern: ~/.*(avi|jpe?g|ogg|mp3|mpe?g|png|wav)/, analysis: Constants.PARSE_BUNDLE],
            ...
            compressed  : [pattern: ~/.*\.(bz2?|gz|z|Z)/, analysis: Constants.PARSE_BUNDLE],                       // revisit this? something better/more efficient?
            web         : [pattern: ~/.*(aspx?|cfm|html?|zhtml?)/, analysis: Constants.PARSE_BUNDLE],
